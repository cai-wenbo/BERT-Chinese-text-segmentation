...
def train(training_config):
    ...
    if os.path.exists(training_config['model_path_src']):
        model = BertTagger.from_pretrained(training_config['model_path_src'])
    else:
        model = BertTagger.from_pretrained("bert-base-chinese", num_labels = 6)
    ...
    for epoch in range(training_config['num_of_epochs']):
        ...
        for i, batch in enumerate(dataloader_train):
            ...
            #  b_label_tensor shape = (batch_size, sequence_length)
            ...
            loss = creterian(outputs.transpose(-2, -1), b_label_tensor).sum()
            ...
            err += (predictions != b_label_tensor).sum().item()
            num_tokens += b_length_tensor.sum().item()
        ...
        for i, batch in enumerate(dataloader_test):
            ...
            with torch.no_grad():
                ...
                loss = creterian(outputs.transpose(-2, -1), b_label_tensor)
                loss_scalar = torch.sum(loss).item()
